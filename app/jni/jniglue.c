...
#ifndef NDEBUG
    __android_log_write(ANDROID_LOG_DEBUG, "openvpn",
            "Loading openvpn native library $id$ compiled on "  __DATE__ " " __TIME__ );
#endif
...
void Java_com_lukekorth_auto_1fi_openvpn_NativeMethods_jniclose(JNIEnv *env, jclass jo, jint fd)
{
	int ret = close(fd);
}

//! Hack to get the current installed ABI of the libraries. See also https://github.com/schwabe/ics-openvpn/issues/391
jstring Java_com_lukekorth_auto_1fi_openvpn_NativeMethods_getNativeAPI(JNIEnv *env, jclass jo)
{
